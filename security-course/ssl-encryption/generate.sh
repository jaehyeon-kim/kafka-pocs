...
if [ ! -e "$TRUSTSTORE_WORKING_DIRECTORY" ]; then
  ...
  openssl req -new -newkey rsa:4096 -days $VALIDITY_IN_DAYS -x509 \
    ...
    -out $TRUSTSTORE_WORKING_DIRECTORY/ca-cert -nodes
...
while read -r KAFKA_HOST || [ -n "$KAFKA_HOST" ]; do
  ...
  keytool -genkey -keystore $KEYSTORE_WORKING_DIRECTORY/"$KEY_STORE_FILE_NAME" \
    -validity $VALIDITY_IN_DAYS -storepass $PASSWORD -keypass $PASSWORD \
    -dname "C=$COUNTRY, ST=$STATE, L=$LOCATION, O=$OU, CN=$KAFKA_HOST" \
    -alias localhost -keyalg RSA -storetype pkcs12 -noprompt
  # keytool -list -v -keystore -keystore $KEYSTORE_WORKING_DIRECTORY/"$KEY_STORE_FILE_NAME"
